...
#Determine if the force exceeds slip
def check_moving(F0, F_z, mu):
    F = abs(F_z * mu)

    F01 = abs(F0)

    if F01 < 0.5:#F:
        #If the tire force has not overcome the static force, don't move
        F0 = 0
...
def formula_longitudinal(kappa, gamma, F_z, p):
    ...
    # magic tire formula
    F_x = D_x * math.sin(C_x * math.atan(B_x * kappa_x - E_x * (B_x * kappa_x - math.atan(B_x * kappa_x))) + S_vx)
