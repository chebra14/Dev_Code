...
from f110_gym.utils.car_parameters import LongitudinalParameters, SteeringParameters, TireParameters
...
class VehicleParameters:
    ...
    #F1Tenth Addition
    Kv: Optional[float] = None  # RPM/V of motor
    MaxRPM: Optional[float] = None  # Maximum RPM of Motor
    Kp_Accl: Optional[float] = None # Speed Controller Acceleration Kp value for PID
    Kp_Brake: Optional[float] = None # Speed Controller Braking Kp value for PID
...
def setup_vehicle_parameters(vehicle_id: int,tire_id: int, dir_params: str = None) -> VehicleParameters:
    """
    ...
    conf_tires = OmegaConf.load(path_root / f'{"parameters_tire"}{tire_id}.yaml')
