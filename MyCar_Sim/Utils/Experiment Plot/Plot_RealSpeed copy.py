...
def main():
    ...
    # array1 = np.array(t_s_2).reshape(-1, 1)
    # array2 = np.array(slip).reshape(-1, 1)
    # array3 = np.array(expected_speed).reshape(-1, 1)
...
def integrate_trapezoidal(values, time_values):
    integrated_values = []

    integrated_values.append(0,0)
    for i in range(1, len(time_values)):
        t1 = time_values[i-1]
        t2 = time_values[i]
        v1 = values[i-1]
        v2 = values[i]
        integral = (v1 + v2) * (t2 - t1) / 2
